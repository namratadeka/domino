...
class RareMixture(GaussianMixture):
    ...
    def fit(self, X, y, y_hat):
        ...
        err_entropy = self.fit_predict(X, y, y_hat)
        return err_entropy
...
def weight_by_entropy(y, y_hat, resp, nk, n_error_classes):
    ...
    if n_error_classes == 2:
        error_class = np.zeros_like(y)
        error_class[:, 1] = np.asarray(pred == true, dtype=int)
        error_class[:, 0] = 1 - error_class[:, 1]
    elif n_error_classes == 4:
        error_class = np.zeros((len(true), 4))
        t0 = np.where(true == 0)[0]
        t1 = np.where(true == 1)[0]
        p0 = np.where(pred == 0)[0]
        p1 = np.where(pred == 1)[0]
        error_class[np.array(list(set(t0).intersection(set(p0)))), 0] = 1
        error_class[np.array(list(set(t0).intersection(set(p1)))), 1] = 1
        error_class[np.array(list(set(t1).intersection(set(p0)))), 2] = 1
