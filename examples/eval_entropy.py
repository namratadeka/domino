...
def _run_slicer(problem, n_slices, n_mixtures, lamda, emb_dp):
    print(problem)
    ...
    for i in range(len(metrics)):
        metrics[i]['err_entropy'] = slicer.err_entropy[metrics[i]['pred_slice_idx']]
    return metrics

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description = 'rare SDM evaluation script, mixtures weighted by error entropy.'
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    outpath = f'./results/entropy4_ent/{args.slice}/k{args.k}_m{args.m}_lambda_{args.lamda}_{args.seed}.csv'
    ...
    for task in tasks:
        try:
            result = _run_slicer(sd.problems[task], args.k, args.m, args.lamda, emb_dp)
